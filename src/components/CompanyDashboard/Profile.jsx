...
const Profile = () => {
  ...
  const dummyCompanyData = {
    ...
    websiteLink: "https://dummycorp.com",
    foundedOn: "2010-01-01T00:00:00.000Z",
    employees: 42,
    locations: ["India", "United States"],
    industry: "Software",
    description: "We build dummy products for demonstration.",
    techStack: ["React", "Node.js", "Python"],
    benefits: [
      { title: "Health Insurance", description: "Comprehensive health plan." },
    ...
    teamMembers: [
      {
        ...
        linkedInLink: "https://linkedin.com/in/alicedoe",
        instagramLink: "https://instagram.com/alicedoe",
      },
      {
        name: "Bob Smith",
        role: "CTO",
        pic: "https://randomuser.me/api/portraits/men/46.jpg",
        linkedInLink: "https://linkedin.com/in/bobsmith",
      },
    ],
  };

  setCompany(dummyCompanyData);
  setCompanyId(dummyCompanyData._id);

  if (dummyCompanyData.companyLogo) {
    setLogoImage(dummyCompanyData.companyLogo);
  }

  setCompanyInfo({
    name: dummyCompanyData.companyName || "",
    website: dummyCompanyData.websiteLink || "",
    founded: dummyCompanyData.foundedOn
      ? new Date(dummyCompanyData.foundedOn).toLocaleDateString("en-US", {
          year: "numeric",
          month: "long",
          day: "numeric",
        })
      : "",
    employees: dummyCompanyData.employees
      ? dummyCompanyData.employees.toString()
      : "",
    location:
      dummyCompanyData.locations && dummyCompanyData.locations.length > 0
        ? dummyCompanyData.locations.join(", ")
        : "",
    industry: dummyCompanyData.industry || "",
    description: dummyCompanyData.description || "",
  });

  // Tech stack
  if (dummyCompanyData.techStack && dummyCompanyData.techStack.length > 0) {
    const tech = dummyCompanyData.techStack.map((item, index) => {
      const randomColor =
  ...
  // Locations
  if (dummyCompanyData.locations && dummyCompanyData.locations.length > 0) {
    const locs = dummyCompanyData.locations.map((loc) => ({
      name: loc,
      logo: getFlagURL(loc),
    }));
    setLocations(locs);
  ...
  // Benefits
  if (dummyCompanyData.benefits && dummyCompanyData.benefits.length > 0) {
    const benefitsData = dummyCompanyData.benefits.map((benefit, index) => {
  ...
  // Team members
  if (dummyCompanyData.teamMembers && dummyCompanyData.teamMembers.length > 0) {
    const team = dummyCompanyData.teamMembers.map((member) => {
      const socialLinks = [];
  ...
  const dummyJobs = [
    {
      ...
      company: dummyCompanyData.companyName,
      location: dummyCompanyData.locations[0],
      jobType: "Full-time",
      categories: ["Full-time"],
      logo: dummyCompanyData.companyLogo,
    },
    {
      title: "Backend Engineer",
      company: dummyCompanyData.companyName,
      location: dummyCompanyData.locations[1],
      jobType: "Remote",
      categories: ["Remote"],
      logo: dummyCompanyData.companyLogo,
    },
