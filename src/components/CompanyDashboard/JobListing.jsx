...
const DUMMY_JOBS = [
  {
    ...
    applicants: 7,
    needs: "1/2",
    description: "Build amazing UI with React.",
    salary: "$90,000 USD"
  },
  {
    id: "2",
  ...
  // ...add more jobs as needed for testing pagination
];
...
const JobListing = () => {
  ...
  // Simulate loading dummy jobs
  useEffect(() => {
    setLoading(true);
    setTimeout(() => {
      setJobs(DUMMY_JOBS);
      // Set date range based on jobs data
      if (DUMMY_JOBS.length > 0) {
        const dates = DUMMY_JOBS.map(job => {
          const [d, m, y] = job.datePosted.split(" ");
