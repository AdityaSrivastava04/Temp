...
const College = () => {
  ...
  const [selectedCollegeForRenewal, setSelectedCollegeForRenewal] = useState(null);
  ...
  const filteredColleges = colleges.filter((college) => {
    ...
    const matchesSubscriptionFilter =
      ...
      selectedSubscriptionFilters.includes(college.subscription.status);
  ...
  const openRenewModal = (college) => {
    setSelectedCollegeForRenewal(college);
    setShowRenewModal(true);
  };

  const closeRenewModal = () => {
    setShowRenewModal(false);
    setSelectedCollegeForRenewal(null);
  };

  const handleRenewalSubmit = () => {
    // Here you would typically make an API call to update the subscription
    alert(`Subscription renewed for ${selectedCollegeForRenewal.name} - ${renewalPlan} plan for ${renewalDuration} year(s)`);
    closeRenewModal();
  };
  ...
  const CollegeCard = ({ college }) => (
    ...
    >
      ...
      <div className="flex justify-between items-center text-[0.65rem] mb-1">
        <span className="text-gray-500">Expires: {college.subscription.expires}</span>
        <span
          className={`px-1.5 py-0.5 rounded-full flex items-center gap-1 ${getSubscriptionTextColor(
            college.subscription.status
          )} ${getSubscriptionColor(college.subscription.status)}`}
        >
          {getSubscriptionIcon(college.subscription.status)}
          {college.subscription.status.replace('_', ' ')}
        </span>
      </div>
  ...
  return (
    <div className="flex flex-col min-h-screen">
                  ...
                  {filterMenuOpen && (
                    <div className="absolute top-full left-0 mt-1 w-56 bg-white border border-gray-200 rounded-md shadow-lg z-10 py-1">
                      ...
                      <div className="px-3 py-1 border-b border-gray-100">
                        <h4 className="text-xs font-medium text-gray-600 mb-1">College Type</h4>
                          ...
                          <td className="p-2">
                            <div className="flex items-center gap-1">
                              ...
                              <span
                                ...
                                ${getSubscriptionTextColor(college.subscription.status)}`}
                              >
                                {college.subscription.type}
                              </span>
                              <span
                                className={`px-1.5 py-0.5 rounded-full text-[0.65rem] flex items-center gap-1 
                                ${getSubscriptionTextColor(college.subscription.status)} 
                                ${getSubscriptionColor(college.subscription.status)}`}
                              >
                                {college.subscription.status.replace('_', ' ')}
      ...
      {showRenewModal && selectedCollegeForRenewal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 cursor-pointer">
          <div 
            className="bg-white rounded-lg shadow-xl w-full max-w-md cursor-default"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="p-4">
              <div className="mb-4">
                <p className="text-sm text-gray-600 mb-2">
                  Renewing subscription for: <span className="font-medium">{selectedCollegeForRenewal.name}</span>
                </p>
                <p className="text-sm text-gray-600">
                  Current plan: <span className="font-medium">{selectedCollegeForRenewal.subscription.type}</span>
                </p>
                <p className="text-sm text-gray-600">
                  Expires on: <span className="font-medium">{selectedCollegeForRenewal.subscription.expires}</span>
                </p>
              </div>
